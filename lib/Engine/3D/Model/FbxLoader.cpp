...
void FbxLoader::LoadModel(fbxModel* model, const std::string& modelDirectory, bool inverseU_, bool inverseV_, bool animeFlag)
{
	...
	model->mScene = aiImportFile(filePath.c_str(), flag);
...
void FbxLoader::ParseMeshFaces(fbxModel* model, aiMesh* mesh)
{
	...
	//インデックス数に応じてメモリを確保
	indices.resize(static_cast<std::vector<uint32_t, std::allocator<uint32_t>>::size_type>(mesh->mNumFaces) * 3);
	...
	for (auto i = 0u; i < mesh->mNumFaces; ++i)
	{
		...
		indices[static_cast<std::vector<uint32_t, std::allocator<uint32_t>>::size_type>(i) * 3] = face.mIndices[0];;
		indices[static_cast<std::vector<uint32_t, std::allocator<uint32_t>>::size_type>(i) * 3 + 1] = face.mIndices[1];
		indices[static_cast<std::vector<uint32_t, std::allocator<uint32_t>>::size_type>(i) * 3 + 2] = face.mIndices[2];
	}
}
